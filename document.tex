...
Podpisy cyfrowe realizowane są w oparciu o kryptografię asymetryczną. Klucz prywatny jest używany do składania podpisu, zaś klucz publiczny służy do weryfikacji. Podpis cyfrowy zapewnia autentyczność. Bezpieczeństwo podpisu zależy od tego czy klucz prywatny jest tajny i klucz publiczny faktycznie należy do nadawcy oraz od bezpieczeństwa używanej funkcji skrótu. 

\textbf{Właściwości bezpieczeństwa informacji} to poufność, integralność, osiągalność, autentyczność, rozliczalność, niezaprzeczalność (CIA - confidentiality, integrity, availability) \cite{Pejas}.

Podpis cyfrowy powinien zapewniać autentyczność pochodzenia, niezaprzeczalność i integralność.

\textbf{Modele bezpieczeństwa} określają jakie problemy bezpieczeństwa występują w systemach informacyjnych. Pierwszym z nim jest \textbf{model ochrony danych w sieci komputerowej} gdzie intruz podsłuchuje kanał komunikacyjny i próbuje odgadnąć przesyłane wiadomości. 

Drugim modelem jest \textbf{model obrony dostępu do sieci komputerowej} gdzie intruz chce dostać się do system informatycznego.
...
Głównym podziałem metod jest podział na \textbf{metody dokładne} oraz \textbf{przybliżone}. 

\textbf{Metody dokładne} to: metoda podstawiania, metoda przeciwnych współczynników, wzory Cramera, metoda eliminacji Gaussa, metoda eliminacji Gaussa-Jordana i metody wykorzystujące rozkłady macierzy.

Metody te dla zadań źle uwarunkowanych mogą dawać duże błędy. Mogą być niestabilne ze względu na błędy zaokrągleń. Zaletą jest mała liczba obliczeń.

\textbf{Metody przybliżone} to: metoda Richardsona, metoda iteracji prostej (metoda Jacobiego), metoda Gaussa-Seidela, metoda nadrelaksacji, metoda najszybszego spadku, metoda gradientów sprzężonych. 

Są to metody iteracyjne, które zbiegają ku rozwiązaniowi układu. Metody te są stabilne, efektywne dla rzadkich i dużych układów.
...
Tworzenie wątku za pomocą instrukcji \lstinline{std::thread t(&thread_function);}. Inną możliwością jest tworzenie wątków przy wykorzystaniu wyrażenia lambda.

Tworzenie obiektu mutex za pomocą instrukcji \lstinline{std::mutex g_pages_mutex;} oraz wykorzystanie mutexu w wątku przy pomocy funkcji \lstinline{std::lock_guard<std::mutex> guard(g_pages_mutex);}.
...
Wiedza na temat percepcji ludzi i ich stylów poznawczych może być wykorzystywana w trakcie \textbf{projektowania aplikacji internetowych.}

Aplikacje mogą być przystosowane do odbiorców zwiększając ich zaangażowanie i motywację do korzystania z serwisów. Jednym z środków do badania wizualnych zachowań użytkowników jest \textbf{okulografia}, która może modelować procesy myślowe człowieka.

Innym zastosowaniem badań z kognitywistyki na rozwój sieci społecznościowych było wykorzystanie konceptu \textbf{grywalizacji}. Jednym z takich rozwiązań są liczby polubień, retweetów itd. na platformach społecznościowych.
...
Do uruchomienia nowego wątku służy funkcja \lstinline{pthread_create} do której przekazujemy wskaźnik na funkcję którą chcemy wykonać oraz parametry wywołania tej funkcji. Możemy oczekiwać na zakończenie wątku przy użyciu \lstinline{pthread_join}, gdzie podajemy identyfikator wątku~\cite{Palkowski_POSIX}.

Zdefiniowanie obiektu wątka \lstinline{pthread_t thread;}. 

Stworzenie wątka \lstinline{rc = pthread_create(&thread, NULL, Function, (void *)funParameter);}.
...
Stworzenie obiektu mutex: \lstinline{pthread_mutex_t  mutex  =  PTHREAD_MUTEX_INITIALIZER;} oraz wykorzystanie mutexów w wątku poprzez założenie blokady \lstinline{pthread_mutex_lock(&mutex);} i zwolnienie blokady \lstinline{pthread_mutex_unlock(&mutex);}. 

Zmienna warunkowa \lstinline{pthread_cond_t  cond  =  PTHREAD_COND_INITIALIZER;} oraz jej wykorzystanie \lstinline{pthread_cond_wait(&cond,  &mutex);}. 

Inne funkcje to czekania na skończenie wątku, anulowanie i wychodzenie z wątku: 

\code{Inne instrukcje z Posix Threads}
...
\begin{enumerate}
    ...
    \item Sporcie - do podnoszenia sprawności zawodników (koordynacji wzrokowo-ruchowej).
\end{enumerate}
...
Regularyzacja to modyfikacja modelu, np. przez dodanie kary za złożoność, która ma na celu zmniejszenie błędu testowego \textbf{zapobiegając zjawisku przeuczenia} \cite{wiki:Regularization}.
